...
class QwenEncoder(nn.Module):
    def __init__(
        ...
        self.model = AutoModelForCausalLM.from_pretrained(pretrain_path, device_map="auto", attn_implementation='sdpa', torch_dtype=self.dtype)
...
class QwenEmbeddingEncoder(nn.Module):
    def __init__(
        ...
        self.model = Qwen2ForCausalLM.from_pretrained(pretrain_path, device_map="auto", attn_implementation='sdpa', torch_dtype=self.dtype)
        self._output_size = self.model.config.hidden_size
...
class QwenInputOnlyEncoder(nn.Module):
    def __init__(
        ...
        if dtype == "fp16":
            ...
            self.dtype = torch.float32
        from transformers import Qwen2ForCausalLM
        model = Qwen2ForCausalLM.from_pretrained(pretrain_path, device_map="auto", attn_implementation='sdpa', torch_dtype=self.dtype)
